...
### Added
- **Words Page**: Отдельная полноэкранная страница для списка слов (вместо вкладки в модалке).
  - Нативный скроллинг для идеальной работы на мобильных.
  - Навигация "Назад" и локализованная статистика ("Выучено"/"В процессе").
- **Sticky Header**: Заголовок теперь всегда виден при скролле (закрпелен сверху).
- **Offline Friends**: Оффлайн друзья теперь отображаются в лобби (серым цветом).
- **Logout Button**: Добавлена кнопка выхода в модальное окно профиля.

### Changed
- **Profile UI**:
  - Модальное окно профиля перемещено наверх (под заголовок) для лучшей видимости.
  - Вкладка "Слова" заменена на кнопку перехода на отдельную страницу.
  - Мгновенная реакция интерфейса при добавлении/удалении друзей (Optimistic UI).
- **Layout Architecture**:
  - Переход от фиксированной высоты (fixed-height flexbox) к нативному скроллу документа (`items-start`, `min-h-screen`).
  - Удалены ограничения `overflow` с `body` для исправления проблем прокрутки.
- **Game Balance**:
  - Количество раундов по умолчанию уменьшено с 15 до 10.
  - Боты: Снижена точность и скорость (~20%) в конфиге `BOTS`.
- **API**:
  - `/api/profile` теперь возвращает историю для всех пользователей (включая ботов).
  - `/api/friends` возвращает полные объекты данных для корректного отображения оффлайн статуса.

### Fixed
- **Scrolling**: Исправлены проблемы со скроллом на мобильных устройствах (переход на native scroll).
- **Leaderboard**: Логика обновлена для корректного отображения топ-30 + ранга текущего игрока.
- **Challenge**: Исправлен баг, когда модальное окно вызова оставалось висеть при отключении соперника.
